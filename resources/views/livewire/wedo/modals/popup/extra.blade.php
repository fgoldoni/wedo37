<div class="bg-white shadow sm:rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        ...
        <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
            <a href="{{ route('carts.index') }}" wire:click="$emit('closeModal')" class="btn-base w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white bg-gradient-to-r from-{{ app_color() }}-500 via-{{ app_color() }}-600 to-{{ app_color() }}-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-{{ app_color() }}-300 dark:focus:ring-{{ app_color() }}-800 sm:col-start-2 sm:text-sm">
                Go to Basket
            </a>
            <button type="button" wire:click="$emit('closeModal')" class="mt-3 btn-base w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-{{ app_color() }}-500 sm:mt-0 sm:col-start-1 sm:text-sm">
                Browse Extras
            </button>
        </div>
    </div>
