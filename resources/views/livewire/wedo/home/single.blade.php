                ...
                <div class="flex flex-col items-center p-10 md:flex-row">
                    <div class="w-full text-white md:w-2/3">
                        <div class="flex items-center">
                            <p class="flex items-start text-lg font-bold leading-none text-transparent text-7xl bg-clip-text bg-gradient-to-r from-{{ $ticket->color }}-100 to-{{ $ticket->color }}-500"><span class="mt-2 text-4xl">€</span>{{ $ticket->price }}</p>
                        </div>
                    </div>

                    <div class="flex items-center w-full mt-8 md:w-2/3 md:justify-end md:mt-0">
                        <a href="{{ route('tickets.show', $ticket->id) }}" class="uppercase hover:scale-105 ease-in-out duration-150 w-full px-10 py-5 text-xl font-semibold text-center text-white rounded-lg bg-gradient-to-br from-{{ $ticket->color }}-100 to-{{ $ticket->color }}-500 md:w-auto">
                            {{ __('layout.single.details') }}
                        </a>
                    </div>
