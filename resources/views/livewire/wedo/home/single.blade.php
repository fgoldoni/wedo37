@if($ticket)
    ...
    <section class="w-full py-20 bg-gray-100">
        ...
        <div class="max-w-3xl px-10 mx-auto"  wire:click="show({{$ticket->id}})">
            <h2 class="uppercase mb-10 text-xl text-3xl font-bold text-center text-transparent bg-clip-text bg-gradient-to-r from-{{ app_color() }}-500 to-{{ app_color() }}-900">
            ...
            <div class="relative flex flex-col overflow-hidden bg-gray-900 rounded-xl">
                ...
                <div class="flex flex-col items-center p-10 md:flex-row">
                    ...
                    <div class="flex items-center w-full mt-8 md:w-2/3 md:justify-end md:mt-0">
                        <a href="{{ route('tickets.show', $ticket->id) }}" class="uppercase hover:scale-105 ease-in-out duration-150 w-full px-10 py-5 text-xl font-semibold text-center text-white rounded-lg bg-gradient-to-br from-{{ app_color() }}-100 to-{{ app_color() }}-500 md:w-auto">
