            ...
            <div class="md:absolute relative lg:w-96 w-full md:max-w-md right-0 top-0 md:py-8 lg:py-12 h-full">
                <div class="w-full h-full flex flex-col items-start justify-between rounded-b-2xl md:rounded-2xl p-8 bg-white shadow-xl">
                    <div class="bg-gray-50 mb-3 lg:flex flex md:hidden border border-gray-100 text-{{ app_color() }}-500 px-3.5 space-x-1 items-center py-1.5 text-xs rounded-full inline-block font-bold">
                        ...
                        <span class="uppercase">{{ __('layout.card.title') }}</span>
                    </div>
                    <div class="relative">
                        <h3 class="uppercase text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-800">{{ __('layout.card.bar') }}</h3>
                        <p class="text-gray-500 text-sm py-5 lg:py-3 lg:text-base">
                            {{ __('layout.card.description') }}
                        </p>
                    </div>
                    <a href="javascript:;" onclick="Livewire.emit('openModal', 'wedo.modals.popup.extras')" class="bg-{{ app_color() }}-500 px-6 py-3 text-sm font-semibold rounded-full text-white">{{ __('layout.card.details') }}</a>
                </div>
            </div>
        </div>
    </div>
