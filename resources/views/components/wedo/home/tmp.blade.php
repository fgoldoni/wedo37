<section class="relative w-full h-auto bg-gradient-to-bl from-slate-900" x-data="{ showMenu: false }" data-tails-scripts="//unpkg.com/alpinejs">
            ...
            <ul class="relative z-10 flex items-center px-6 text-sm text-white lg:text-base">
                <li class="mx-2 lg:mx-3">
                    <a href="{{ route('jobs.index') }}" x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false" class="relative inline-block font-medium text-gray-200 hover:text-white">
                        <span class="block">{{ __('pages/home.hero.browse_jobs') }}</span>
                ...
                @else
                    <li class="mx-2 lg:mx-3">
                        <a href="{{ route('login') }}" x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false" class="relative inline-block font-medium text-gray-200 hover:text-white">
                            <span class="block">{{ __('pages/home.hero.sign_in') }}</span>
                            <span class="absolute bottom-0 left-0 inline-block w-full h-1 -mb-1 overflow-hidden">
                                <span x-show="hover" class="absolute inset-0 inline-block w-full h-1 h-full transform border-t-2 border-{{ app_color() }}-500" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-out duration-300" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" style="display: none;"></span>
                            </span>
                        </a>
    ...
    <div x-show.transition="showMenu" class="absolute top-0 z-20 flex flex-col items-center justify-center w-full h-full space-y-5 text-lg origin-center bg-{{ app_color() }}-500" style="display: none;">
        <a href="{{ route('jobs.index') }}" class="block text-{{ app_color() }}-200 hover:text-white">{{ __('pages/home.hero.browse_jobs') }}</a>
        @auth
            <a href="{{ route('accounts.index') }}" class="block text-{{ app_color() }}-200 hover:text-white">{{ __('pages/home.hero.personal_account') }}</a>
            <a href="{{ route('resumes.index') }}" class="block text-{{ app_color() }}-200 hover:text-white">{{ __('pages/home.hero.resumes') }}</a>
            <a href="{{ route('applicants.index') }}" class="block text-{{ app_color() }}-200 hover:text-white">{{ __('pages/home.hero.applications') }}</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <a class="block text-{{ app_color() }}-200 hover:text-white cursor-pointer" href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();">
                    {{ __('pages/home.hero.sign_out') }}
                </a>
            </form>
        @else
            <a href="{{ route('login') }}" class="block text-{{ app_color() }}-200 hover:text-white"> {{ __('pages/home.hero.sign_in') }}</a>
        @endauth
    </div>
    <div class="container flex items-center justify-center h-auto py-56 mx-auto">
        <div class="z-10 flex flex-col items-center xl:px-0 space-y-20">
            <h1 class="mx-6 mt-1 text-2xl text-center text-white sm:text-center sm:mx-0">{{ __('pages/home.hero.welcome') }}
                <br/><br/> <span class="text-5xl sm:text-6xl font-semibold">{{ __('pages/home.hero.header', ['team' => app_team_name()]) }}</span>
            </h1>
            <div class="flex justify-center md:mt-10">
                <button type="button" onclick="Livewire.emit('openModal', 'wedo.modals.popup.wedo')" class="uppercase px-8 py-2 m-2 text-center rounded-full text-white  text-sm sm:text-base bg-{{ app_color() }}-500 border-2 border-{{ app_color() }}-500 hover:bg-transparent">{{ __('pages/home.hero.actions.application') }}</button>
                <a href="{{ route('jobs.index') }}" class="uppercase px-8 py-2 m-2 text-center rounded-full text-white border-2 text-sm sm:text-base border-{{ app_color() }}-500 hover:bg-{{ app_color() }}-500">{{ __('pages/home.hero.actions.browse_jobs') }}</a>
            </div>
        </div>
