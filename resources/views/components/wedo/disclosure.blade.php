...
<div {{ $attributes->merge(['class' => 'w-full mx-auto px-4 sm:px-6 lg:px-8']) }}>
    ...
    <div wire:ignore x-data="{
        ...
        <div class="col-span-1 sm:col-span-3" id="disclosure-1" x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-90">
            {{ $slot }}
        </div>
    </div>
</div>
